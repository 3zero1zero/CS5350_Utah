...
def back_propagation_stochastic_random(train, test, l_rate, d, n_epoch, n_hidden):
    ...
    if (n_outputs > 0):
        network = neuralNet.initialize_network_random([n_inputs, n_hidden, n_outputs])
    else:
        print ("outputs error")
    neuralNet.train_network(network, train, l_rate, d, n_epoch, n_outputs)
...
def back_propagation_stochastic_zeros(train, test, l_rate, d, n_epoch, n_hidden):
    ...
    n_outputs = len(set([row[-1] for row in train]))
    if (n_outputs > 0):
        network = neuralNet.initialize_network_zeros([n_inputs, n_hidden, n_outputs])
    else:
        print ("outputs error")
    neuralNet.train_network(network, train, l_rate, d, n_epoch, n_outputs)
    for i in range (length):
...
def predict(network, row):
    if (row != ""):
        out = neuralNet.forward_pass(network, row)
...
def main():
    ...
    for width in n_hidden:
        if (width > 0):
            p, p2 = back_propagation_stochastic_zeros(dataset, dataset2, l_rate, d, n_epoch, width)
        else:
            print ("width need to be bigger than zero")
        error_train = get_error(np.array(dataset), p)
        error_test = get_error(np.array(dataset2), p2)
